    ...
    <>
      <Fade in={isOpen}>
        <Paper 
          elevation={24}
          sx={{
            position: 'fixed', bottom: { xs: 0, sm: 90 }, right: { xs: 0, sm: 32 },
        ...
        >
          ...
          <Box sx={{ 
            p: 3, 
            background: 'linear-gradient(135deg, #2C3E50 0%, #4CA1AF 100%)', // Slate to Teal gradient
            color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            boxShadow: '0 4px 20px rgba(0,0,0,0.1)'
          }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Box sx={{ position: 'relative' }}>
                <Avatar sx={{ bgcolor: 'white', color: '#2C3E50', width: 44, height: 44 }}>
                ...
                <Box key={idx} sx={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', mb: 2 }}>
                  ...
                  {isSummary ? (
                                ...
                                if (line.includes(':')) {
                                    ...
                                    return (
                                        <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1, fontSize: '0.9rem' }}>
                                            <Typography variant="caption" color="text.secondary" fontWeight="bold">{key.trim().toUpperCase()}</Typography>
                                            <Typography variant="body2" fontWeight="600" color="#2D3748" align="right">{val?.trim()}</Typography>
                                        </Box>
                                    );
                                }
                                return <Typography key={i} variant="body2" sx={{ mt: 1, fontStyle: 'italic', color: '#718096' }}>{line}</Typography>;
                            })}
                        </Box>
                    ...
                    <Paper 
                      ...
                      sx={{ 
                        p: 2, px: 2.5, maxWidth: '80%', 
                        bgcolor: isUser ? '#2C3E50' : 'white',
                    ...
                    >
                      <Typography variant="body2" sx={{ lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: msg.text.replace(/\n/g, '<br/>') }} />
                    </Paper>
                  )}
                </Box>
              );
